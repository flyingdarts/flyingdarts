...
use flyingdarts_auth::{
    create_auth_response, create_auth_response_with_user, create_default_authress_service, AuthService,
    ExampleAuthService,
};
...
pub async fn authress_handler(event: LambdaEvent<ApiGatewayProxyRequest>) -> Result<ApiGatewayProxyResponse, Error> {
    let request = event.payload;
    ...
    let auth_header = match request.headers.get("Authorization") {
        ...
        None => {
            return Ok(create_api_response(401, Some("Missing Authorization header".into())));
        }
    };
...
mod tests {
    ...
    fn test_auth_service_integration() {
        ...
        assert!(!is_authorized);
        assert_eq!(auth_response.message, "You are trespassing, leave immediately! 😂");
    }
}
